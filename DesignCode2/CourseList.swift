...
struct CourseList: View {
    ...
    var body: some View {
        ZStack {
            Color.gray.opacity(Double(self.activeView.height / 500))
                .animation(.linear)
...
//            )
            ...
            //调用一个新的可重用视图，并保证动画平滑,并且可滚动，但是，目前有些问题
            //            if show {
        ...
        .rotation3DEffect(Angle(degrees: Double(self.activeView.height / 50)), axis: (x: 10, y: 0.0, z: 0))
        .animation(.spring(response: 0.5, dampingFraction: 0.6, blendDuration: 0))
        .gesture(
            //判断，激活状态下，拖拽才生效
            show ?
